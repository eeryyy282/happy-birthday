...
function initPhotosInteractive() {
    ...
    cards.forEach(card => {
        card.addEventListener('click', () => {
            ...
            if (!isActive) card.classList.add('is-touched');
        });
    });
}
initPhotosInteractive();
...
function nyalakanLilin() {
    ...
    setTimeout(() => { instruction.style.opacity = '1'; instruction.style.transform = 'translateY(0)'; }, 600);
}
...
function tiupLilin() {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            // 1. Munculkan wadahnya (namun secara CSS opacity-nya masih 0, jadi tetap tak terlihat)
            hiddenArea.style.display = 'block';

            // 2. Force DOM Reflow agar browser merender perubahan display:block di atas sebelum transisi jalan
            void hiddenArea.offsetWidth;

            // Request Animation Frame agar rendering browser lebih stabil
            requestAnimationFrame(() => {
                // 3. Lakukan Scroll mulus ke elemen
                hiddenArea.scrollIntoView({ behavior: 'smooth', block: 'start' });

                // 4. Beri waktu 800ms sampai scroll layar nyaris berhenti, lalu munculkan gambar perlahan
                setTimeout(() => {
                    hiddenArea.classList.add('revealed'); // Ini akan memicu opacity: 1 dari CSS dengan mulus
                }, 800);
            });
